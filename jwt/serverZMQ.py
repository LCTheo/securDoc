...
class ThreadVerifToken(Thread):
	...
	#
	#################################################################
	def __init__(self, ctx,portReceive, portSend,serversecretkeyfile, clientpublickeyfile, secretkey, filename, ipClient):
	...
	def run(self):
		...
		serverReceive = initReceive(self.ctx,self.portReceive,self.serversecretkeyfile,self.clientpublickeyfile,self.filename,self.ipClient)
		mes=""
		while True :
...
class ThreadCreateToken(Thread):
	...
	def run(self):
		...
		while True :
			if serverReceive.poll(1000):
				msg = serverReceive.recv_string()
				print(msg)
				print(self.createJWT(msg,self.secretkey))
				serverSend.send_string(self.createJWT(msg,self.secretkey))
	...
	def createJWT(self,payload,secret):
		...
		ID = tab[0]
		hash_password = tab[1]
		payload = {'id':ID,'hash_password':hash_password, 'exp': datetime.datetime.utcnow() + datetime.timedelta(minutes=15)};
